...
setup_router() {
  ...
  docker exec "$container_id" sh -c "
    ...
    ip link add name vxlan10 type vxlan id 10 dev eth0 remote $remote_ip local $local_ip dstport 4789
...
main() {
  for container_id in $(docker ps -q); do
    ...
    "$host2")
      ...
      ;;
    # arguments : container, local ip, remote ip
    "$router1")
      setup_router $container_id "10.1.1.1" "10.1.1.2"
      ;;
    "$router2")
      setup_router $container_id "10.1.1.2" "10.1.1.1"
      ;;
    *)
